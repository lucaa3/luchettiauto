...
function renderVehicle() {
  ...
  if (km) {
    const rawNumber = Number(v.km)

    if (!isNaN(rawNumber)) {
      km.textContent = rawNumber.toLocaleString("it-IT") + " km"
    } else {
      // Fallback: If they typed something like "TBA" or "N/A", just print the raw text
      km.textContent = v.km
    }
  }
  if (price) {
    ...
    if (!isNaN(rawNumber)) {
      ...
      // Fallback: If it's not a number, just print the raw text
...
function renderDots() {
  ...
  vehicles.forEach((_, index) => {
    ...
    dot.type = "button"
    dot.className =
      index === currentVehicleIndex ? "vehicle-dot active" : "vehicle-dot"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (vehicleImage) {
    ...
    vehicleImage.addEventListener("dragstart", (event) =>
